...
public class PerformanceFractalDrawer implements FractalRender {
	...
	public PerformanceFractalDrawer (Fractal f, ColorMap color, int iterations, double x1, double y1, double x2, double y2, int width, int height){
		this.fractal = f;
		this.color = color;
	...
	public BufferedImage DrawFractal(Fractal f, ColorMap color, int iterations, double x1, double y1, double x2, double y2, int width, int height){
		for(int x = 0; x < width; x++){
	    	for(int y = 0; y < height; y++){
	    		imgarray[(y * width) + x] = color.getColor(this.getPixel(x, y));
	    	}
	    }
